...
Status: deferred until operational backbone is proven.

Definition of done:

- `astro propagate --backend tudat` supports at least one cross-check scenario and records Tudat provenance.
- `astro research-propagate --backend jax` runs seeded batch propagation or sensitivity experiments without replacing operational Orekit semantics.
- Nyx/ANISE evaluation has a documented spike result and a yes/no decision for a production adapter.
