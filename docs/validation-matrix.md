...
| RocketPy-configured launch schema | `python -m pytest tests/astro_launch/test_launch_io.py::test_load_rocketpy_configured_launch_scenario -q` | Loads `examples/launch/rocketpy_configured_two_stage.yaml` with typed RocketPy vehicle/motor/flight configuration. |
...
| RocketPy launch adapter | `astro launch examples/launch/rocketpy_configured_two_stage.yaml --backend rocketpy --output /tmp/astro-rocketpy-launch.json` | Requires a validated RocketPy flight runner and returns suite `LaunchTrajectory` products through the adapter boundary. | Exits 2 with structured `UnsupportedBackendError` diagnostics when a validated runner is not configured. |
