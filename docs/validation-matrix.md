...
| Orekit propagation | `astro propagate examples/scenarios/leo_two_body.yaml --backend orekit --output /tmp/astro-orekit-trajectory.json`, `astro propagate examples/scenarios/leo_j2.yaml --backend orekit --output /tmp/astro-orekit-j2.json`, and `astro propagate examples/scenarios/leo_orekit_high_fidelity.yaml --backend orekit --output /tmp/astro-orekit-high-fidelity.json` | Writes suite `Trajectory` with `backend = "orekit"` for two-body, J2, and high-fidelity numerical-path scenarios. | Exits 2 with `UnsupportedBackendError` diagnostics, including explicit unsupported drag/SRP/third-body flag names. |
