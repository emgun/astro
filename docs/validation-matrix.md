...
| Orekit propagation | `astro propagate examples/scenarios/leo_two_body.yaml --backend orekit --output /tmp/astro-orekit-trajectory.json`, `astro propagate examples/scenarios/leo_j2.yaml --backend orekit --output /tmp/astro-orekit-j2.json`, `astro propagate examples/scenarios/leo_orekit_high_fidelity.yaml --backend orekit --output /tmp/astro-orekit-high-fidelity.json`, `astro propagate examples/scenarios/leo_orekit_drag.yaml --backend orekit --output /tmp/astro-orekit-drag.json`, and `astro propagate examples/scenarios/leo_orekit_srp.yaml --backend orekit --output /tmp/astro-orekit-srp.json` | Writes suite `Trajectory` with `backend = "orekit"` for two-body, J2, high-fidelity numerical-path, atmospheric-drag, and SRP scenarios. | Exits 2 with `UnsupportedBackendError` diagnostics, including explicit unsupported third-body flag names. |
