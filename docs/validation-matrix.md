...
| Orekit native OD bridge | Unit tests map suite WGS-84 geodetic range/range-rate records into Orekit `Range`/`RangeRate` objects, construct a native `BatchLSEstimator` boundary with a numerical propagator builder, execute the estimator through the runtime abstraction, and map estimated state, residuals, RMS, covariance, and iteration diagnostics into `EstimateResult`. |
