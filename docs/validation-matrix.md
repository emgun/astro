...
| Radiometric measurement synthesis | `astro synth-measurements examples/scenarios/leo_doppler.yaml --backend local --output /tmp/astro-doppler-measurements.json` | Writes mixed range and one-way Doppler records, preserving Doppler values in Hz with carrier-frequency metadata. |
...
| OD explicit measurements | Tests validate JSON, CSV, TDM range/range-rate/angle ingest/export, JSON/CSV one-way Doppler preservation, inertial and local-horizon angle generation, WGS-84 geodetic station measurement generation, fixed and tabulated EOP geodetic station transforms, angle wrapping, Doppler residual prediction, and local least-squares convergence. |
