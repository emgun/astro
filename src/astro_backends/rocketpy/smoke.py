from __future__ import annotations

from dataclasses import asdict, dataclass
from importlib.metadata import PackageNotFoundError
from typing import cast

from astro_backends.rocketpy.runtime import (
    PACKAGE,
    RocketPyRuntimeUnavailable,
    load_rocketpy_runtime,
)


@dataclass(frozen=True)
class RocketPySmokeResult:
    available: bool
    package: str
    version: str | None
    message: str

    def to_dict(self) -> dict[str, bool | str | None]:
        return cast(dict[str, bool | str | None], asdict(self))


def _not_installed_result(package_version: str | None = None) -> RocketPySmokeResult:
    return RocketPySmokeResult(
        available=False,
        package=PACKAGE,
        version=package_version,
        message="RocketPy is not installed.",
    )


def run_rocketpy_smoke(
    *,
    strict: bool = False,
    force_unavailable: bool = False,
) -> RocketPySmokeResult:
    if force_unavailable:
        return _not_installed_result()

    try:
        runtime = load_rocketpy_runtime(strict=strict)
    except PackageNotFoundError:
        if strict:
            raise
        return _not_installed_result()
    except ImportError as exc:
        if strict:
            raise
        return RocketPySmokeResult(
            available=False,
            package=PACKAGE,
            version="unknown",
            message=f"RocketPy import failed: {exc}",
        )
    except RocketPyRuntimeUnavailable as exc:
        message = str(exc)
        if "not installed" in message:
            return _not_installed_result(exc.package_version)
        return RocketPySmokeResult(
            available=False,
            package=PACKAGE,
            version=exc.package_version,
            message=message,
        )

    return RocketPySmokeResult(
        available=True,
        package=PACKAGE,
        version=runtime.package_version,
        message="RocketPy Environment, SolidMotor, Rocket, and Flight APIs are available.",
    )
