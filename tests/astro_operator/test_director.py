...
def test_verifier_reconstructs_observations_and_manifest_roles(tmp_path: Path) -> None:
    ...
    selected = next(
        ...
        if step.get("observation", {}).get("candidate", {}).get("candidate_id")
        == "higher-reserve"
    )
    metric = next(
        item
...
def test_planner_rejects_unregistered_capability_contract() -> None:
    ...
    unregistered = capability.model_copy(
        update={
            "capability_id": "evil.unregistered",
            "version": "999",
            "output_metric_ids": ("evil.metric",),
