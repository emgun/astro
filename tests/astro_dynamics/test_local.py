...
def test_propagate_local_supports_two_body_variational_covariance_model() -> None:
    scenario = load_scenario(Path("examples/scenarios/leo_two_body.yaml"))
    initial_covariance = [
        [1.0 if row == column else 0.0 for column in range(6)] for row in range(6)
    ]
    ...
    variational_scenario = Scenario.model_validate(
        ...
        | {
            ...
            "covariance_state_transition_model": "two_body_variational",
        }
    )
    ...
    assert variational.metadata["covariance_model"] == "two_body_variational"
    ...
    assert variational.covariance_history[-1].metadata["variational_dynamics"] == (
        "two_body_acceleration_jacobian"
    )
    assert "finite_difference_relative_step" not in variational.covariance_history[-1].metadata
    assert variational.covariance_history[-1].metadata["transition_step_s"] == (
