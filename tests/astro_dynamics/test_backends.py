...
def test_propagate_with_backend_dispatches_tudat(monkeypatch: pytest.MonkeyPatch) -> None:
    scenario = load_scenario(Path("examples/scenarios/leo_two_body.yaml"))

    def fake_tudat() -> object:
        trajectory = propagate_local(scenario)
        return trajectory.model_copy(update={"backend": "tudat"})

    monkeypatch.setattr("astro_dynamics.backends.propagate_tudat", lambda _scenario: fake_tudat())

    trajectory = propagate_with_backend(scenario, "tudat")
