...
def test_validate_optional_artifact_passes_when_missing(tmp_path) -> None:
    artifact = WorkflowArtifact(
        path=str(tmp_path / "missing.json"),
        kind=ArtifactKind.ESTIMATE_JSON,
        required=False,
...
def test_validate_json_artifact_fails_when_json_is_invalid(tmp_path) -> None:
    path = tmp_path / "estimate.json"
...
def test_validate_non_json_artifact_requires_only_existence(tmp_path) -> None:
